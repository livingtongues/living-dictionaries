...
CREATE TABLE sentences (
  ...
  text_id uuid REFERENCES texts ON DELETE CASCADE, -- if part of a text
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  created_by uuid NOT NULL DEFAULT auth.uid() REFERENCES auth.users,
  updated_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_by uuid NOT NULL DEFAULT auth.uid() REFERENCES auth.users,
...
CREATE TABLE videos (
  ...
  text_id uuid REFERENCES texts ON DELETE CASCADE,
  source text,
  videographer text,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  created_by uuid NOT NULL DEFAULT auth.uid() REFERENCES auth.users,
...
CREATE TABLE content_updates (
  ...
  dictionary_id text NOT NULL REFERENCES dictionaries ON DELETE CASCADE,
  entry_id text REFERENCES entries ON DELETE CASCADE,
  sense_id uuid REFERENCES senses ON DELETE CASCADE,
  sentence_id uuid REFERENCES sentences ON DELETE CASCADE,
  text_id uuid REFERENCES texts ON DELETE CASCADE,
  audio_id uuid REFERENCES audio ON DELETE CASCADE,
  ...
  dialect_id uuid REFERENCES dialects ON DELETE CASCADE,
