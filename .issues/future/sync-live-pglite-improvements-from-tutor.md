# Sync live-pglite improvements from tutor repo
...
Changes needed:
- `packages/old-site/src/lib/pglite/live/types.ts` - rename `delete_all` to `delete` in `TableAccessor`, change param to `string | string[]`
- `packages/old-site/src/lib/pglite/live/live-pglite.svelte.ts` - rename `delete_all` method in accessor and `#delete_all` private method to `#delete`, add `Array.isArray` normalization
...
Changes needed:
- `packages/old-site/src/lib/pglite/live/types.ts` - add `find` to `TableAccessor`
- `packages/old-site/src/lib/pglite/live/live-pglite.svelte.ts` - add `find` method to accessor
...
Changes needed:
- `packages/old-site/src/lib/pglite/live/types.ts` - add `upsert` to `TableAccessor`, add `UpdateType` 
- `packages/old-site/src/lib/pglite/live/live-pglite.svelte.ts` - add `upsert` method and `#upsert` private implementation
...
Changes needed:
- `packages/old-site/src/lib/pglite/live/types.ts` - add `UpdateType` and `update` to `TableAccessor`
- `packages/old-site/src/lib/pglite/live/live-pglite.svelte.ts` - add `update` method and `#update` private implementation
...
Changes needed:
- `packages/old-site/src/lib/pglite/live/types.ts` - add `snapshot` to `QueryAccessor`, import `RawRowType`
- `packages/old-site/src/lib/pglite/live/live-pglite.svelte.ts` - add `snapshot` to `#create_query_accessor` return
...
Changes needed:
- `packages/old-site/src/lib/pglite/live/table-store.svelte.ts`:
...
1. `packages/old-site/src/lib/pglite/live/types.ts` - add `find`, `upsert`, `update`, `UpdateType`, rename `delete_all` → `delete`, add `snapshot` to `QueryAccessor`
2. `packages/old-site/src/lib/pglite/live/live-pglite.svelte.ts` - implement `find`, `upsert`, `update`, rename `delete_all` → `delete`, add `snapshot`
3. `packages/old-site/src/lib/pglite/live/table-store.svelte.ts` - add debounced stop
