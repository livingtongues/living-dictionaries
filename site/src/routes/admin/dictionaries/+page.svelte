<script lang="ts">
  ...
  const active_section = $derived($page.url.searchParams.get('filter') as 'public' | 'private' | 'other')

  const invites = writable<Tables<'invites'>[]>([])

  const dictionaries_with_editors_invites = $derived($admin_dictionaries
