 ...
 * Instead it fetches the daily analytics CHECKPOINT (shared with /admin/analytics — same payload, different panels) via the admin-gated API endpoint
...
export const load: PageLoad = async ({ fetch, parent, url }) => {
  ...
  async function fetch_checkpoint() {
    ...
    if (err || !data)
      throw new Error(err?.message ?? 'Failed to load site health')
    return data
  }
