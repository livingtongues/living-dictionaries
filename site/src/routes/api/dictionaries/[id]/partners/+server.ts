...
import { is_secure_dictionary } from '$lib/db/server/secure-dictionary'
 ...
 * tab, print/PDF citation). The logo is denormalized on the row. Secure
...
export const POST: RequestHandler = async (event) => {
  ...
  try {
    switch (body.action) {
      ...
      case 'delete': {
        ...
        db.prepare('DELETE FROM dictionary_partners WHERE id = ? AND dictionary_id = ?')
          .run(body.partner_id, dictionary.id)
        return json({ result: 'success' } satisfies DictionariesIdPartnersResponseBody)
      }
      case 'set_photo': {
        if (!body.partner_id || !body.photo_storage_path)
    ...
    log_server_event({ level: 'error', message: 'dictionary_partners_update_failed', error: err, context: { dictionary_id: dictionary.id, action: body.action } })
