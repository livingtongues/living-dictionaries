...
import { attach_attachments, attach_reactions, attach_reply_previews, edit_message } from '$lib/server/chat/chat-db'
import { ResponseCodes } from '$lib/constants'
import { error, json } from '@sveltejs/kit'

export interface ChatEditRequestBody {
  message_id: string
  body_html: string
  body_text: string
}
...
export const POST: RequestHandler = async (event) => {
  ...
  const body = await event.request.json() as ChatEditRequestBody
  if (!body.message_id)
  ...
  try {
    ...
    const [message] = attach_reply_previews({ db, messages: attach_reactions({ db, messages: attach_attachments({ db, messages: [edited] }) }) })
