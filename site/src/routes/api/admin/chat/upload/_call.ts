import type { AdminChatUploadResponse } from './+server'

interface CallReturn {
  data: AdminChatUploadResponse | null
 ...
 */
export async function api_admin_chat_upload({ message_id, files }: { message_id: string, files: File[] }): Promise<CallReturn> {
  try {
    const form_data = new FormData()
    form_data.append('message_id', message_id)
    for (const file of files)
      form_data.append('files', file)
    const response = await fetch('/api/admin/chat/upload', { method: 'POST', body: form_data })
    if (!response.ok) {
    ...
    return { data: await response.json() as AdminChatUploadResponse, error: null }
  } catch (err) {
