...
import { actor_label, actor_name, append_import_request_followup, is_site_admin_user, list_import_requests, require_import_request_owner } from '$lib/import/server/import-request-thread'
...
export const PATCH: RequestHandler = async (event) => {
  ...
  const thread = require_import_request_owner({ db, dictionary_id: dictionary.id, thread_id, access })
  if (thread.started_at && !is_site_admin_user({ db, user_id: access.user_id }))
    error(ResponseCodes.FORBIDDEN, 'We have started work on this import, so the request is now part of the dictionary\'s permanent record. Post in the import conversation instead.')
  ...
  const update = db.transaction(() => {
    ...
    const { assigned_email: email } = append_import_request_followup({
      ...
      access,
      body_text: [
        `${actor_name({ db, user_id: access.user_id })} updated the request note.`,
        '',
  ...
  void notify_admin({
    ...
    link: `${event.url.origin}/${dictionary.url}/import/${thread_id}`,
  })
