<script lang="ts">
  ...
  const { dialects: dictionary_dialects, writes } = $derived(page.data)
  ...
  async function on_update(new_values: string[]) {
    ...
    for (const dialect_id of dialect_ids) {
      ...
      if (value_is_removed) {
        await writes.assign_dialect({ dialect_id, entry_id, remove: true })
      }
    }
    ...
    for (const dialect_id of new_values) {
      ...
      if ($dictionary_dialects.find(({ id }) => id === dialect_id)) {
        ...
        await writes.assign_dialect({ dialect_id, entry_id })
      } else {
        // if a value is not in the dictionary's dialects first add the dialect to the dictionary
        const data = await writes.insert_dialect({ name: { default: dialect_id } })
        await writes.assign_dialect({ dialect_id: data.id, entry_id })
      }
    }
  }
</script>
