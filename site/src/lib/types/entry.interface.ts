...
type EntryDataSense = Pick<Tables<'senses'>, 'id' | 'updated_at' | 'created_at' | 'definition' | 'glosses' | 'noun_class' | 'parts_of_speech' | 'plural_form' | 'semantic_domains' | 'sources' | 'variant' | 'write_in_semantic_domains'> & {
  ...
  photos?: Pick<Tables<'photos'>, 'id' | 'updated_at' | 'photographer' | 'storage_path' | 'source' | 'latitude' | 'longitude' | 'taken_at'>[]
...
export interface EntryData {
  id: string
  // `review` is EDITOR-ONLY + OPTIONAL — present only when the viewer can_edit (stripped in assemble_entry_data).
  main: Pick<Tables<'entries'>, 'citations' | 'coordinates' | 'elicitation_id' | 'homograph' | 'lexeme' | 'interlinearization' | 'linguistic_history' | 'morphology' | 'notes' | 'phonetic' | 'scientific_names' | 'sources'> & Partial<Pick<Tables<'entries'>, 'review'>>
