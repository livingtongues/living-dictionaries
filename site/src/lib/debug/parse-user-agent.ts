...
export function parse_user_agent(ua: string | null | undefined): ParsedUserAgent {
  if (!ua)
    return { browser: 'Other', major: null, version: null, os: 'Other' }
  ...
  for (const rule of BROWSER_RULES) {
    ...
    if (match) {
      ...
      return { browser: rule.name, major: Number.isNaN(major) ? null : major, version, os }
    }
  }
  return { browser: 'Other', major: null, version: null, os }
}
...
if (import.meta.vitest) {
  ...
  describe(parse_user_agent, () => {
    it('desktop Safari on Catalina (Wayne)', () => {
      ...
      expect(p).toEqual({ browser: 'Safari', major: 15, version: '15.6', os: 'macOS' })
    })
    ...
    it('iPhone Safari', () => {
      ...
      expect(p).toEqual({ browser: 'Safari', major: 18, version: '18.5', os: 'iOS' })
    })
    ...
    it('Chrome on Linux (Jacob dev)', () => {
      ...
      expect(p).toEqual({ browser: 'Chrome', major: 148, version: '148.0', os: 'Linux' })
    })
    ...
    it('null / empty UA', () => {
      expect(parse_user_agent(null)).toEqual({ browser: 'Other', major: null, version: null, os: 'Other' })
    })
  })
  ...
  describe(is_below_db_worker_capability, () => {
    ...
    it('Safari 15.2 is BELOW (no BroadcastChannel)', () => {
      expect(is_below_db_worker_capability({ browser: 'Safari', major: 15, version: '15.2', os: 'macOS' })).toBe(true)
    })
    it('Safari 14 is BELOW', () => {
      expect(is_below_db_worker_capability({ browser: 'Safari', major: 14, version: '14.1', os: 'macOS' })).toBe(true)
    })
    it('Chrome is never flagged', () => {
      expect(is_below_db_worker_capability({ browser: 'Chrome', major: 100, version: '100.0', os: 'Linux' })).toBe(false)
    })
  })
}
