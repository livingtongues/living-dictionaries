...
const GUIDE_DESCRIPTIONS: Record<string, string> = {
  ...
  'pdf-scans': 'Printed dictionaries as PDF. FIRST tell a scan from a born-digital export: for a scan, why NOT to use OCR (read every page with vision instead, zooming in on uncertain diacritics); for a born-digital file, use the real text layer but decode the custom-font glyph trap (incl. ASCII imposters), fix spacing at the character-geometry level, resolve wrap hyphens by corpus evidence, apply NFC, and verify the parse with a compare-don\'t-retranscribe vision sweep. Plus the print-typo policy, front-matter legend, page-range workflow, diacritic fidelity, per-page citations.',
