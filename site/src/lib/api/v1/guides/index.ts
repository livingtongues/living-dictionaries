...
const GUIDE_DESCRIPTIONS: Record<string, string> = {
  'importing': 'START HERE for any import job: workflow, triaging whether a resource is a real source (registry row + file linking), batching, idempotency, verification, reporting.',
...
if (import.meta.vitest) {
  describe(list_guides, () => {
    test('every guide has a manifest description and an h1 title', () => {
      ...
      expect(guides.map(guide => guide.slug)).toEqual(['importing', 'spreadsheets', 'flex-lift', 'pdf-scans'])
