...
const GUIDE_DESCRIPTIONS: Record<string, string> = {
  ...
  'pdf-scans': 'Printed dictionaries as PDF. FIRST tell a scan from a born-digital export: for a scan, why NOT to use OCR (read every page with vision instead, zooming in on uncertain diacritics); for a born-digital file, use the real text layer but decode the custom-font glyph trap that silently deletes a language\'s special letters. Plus front-matter legend, page-range workflow, diacritic fidelity, per-page citations.',
