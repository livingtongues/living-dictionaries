  ...
  const spec = {
    ...
    info: {
      ...
      description: [
        ...
        'When a dictionary team asks us to import their materials, the original files live at `GET …/files` (write scope) — each with the uploader\'s per-file `import_instructions` (authoritative) and an optional `source_note`. Download originals via `GET …/files/{fileId}`. **Before importing anything, read the workflow guide**: `/api/v1/guides/importing` is the mandatory starting point — it prescribes a data-preparation phase (inspect, question the human, stage locally, review the data by eye, preview sign-off) BEFORE any API write, then the API-usage phase. `GET /api/v1/guides` lists it plus the format guides (spreadsheets, flex-lift, pdf-scans). Register a `sources` row for every import. Only after the data passes verification, link every imported file to the proper source (`PATCH …/files/{fileId}` with `source_id`); that completion marker removes it from the active Import queue and makes it downloadable beneath the source on the manager-only Sources page.',
        '',
