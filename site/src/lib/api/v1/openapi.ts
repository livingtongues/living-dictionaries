...
export function build_openapi_spec({ origin }: { origin: string }): Record<string, unknown> {
  ...
  const OrthographyInput = {
    ...
    properties: {
      ...
      notes: { type: 'string' },
      characters: { type: 'array', items: { type: 'string' }, description: 'Characters of this writing system a normal keyboard cannot type (`["đ","ʼ","ə","ą́"]`). They render as tap-buttons beside the entries search box so anyone can type the language. An item may be a multi-codepoint grapheme (`ą́` = `ą` + U+0301). Max 40.' },
    },
  }
  ...
  const spec = {
    ...
    paths: {
      ...
      '/api/v1/dictionaries/{id}/orthographies/{code}': {
        patch: { summary: 'Relabel an orthography', description: 'Edits `name` / `bcp` / `notes` / `characters` (the `code` itself never changes). Works on the primary too — PATCH `default` to label the main headword, attach a keyboard, or register the special characters shown as tap-buttons beside search. `characters` replaces the whole list; `[]` or `null` clears it.', parameters: [dict_id_param, orthography_code_param], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { name: { type: 'string' }, bcp: { type: 'string', nullable: true }, notes: { type: 'string', nullable: true }, characters: { type: 'array', items: { type: 'string' }, nullable: true, description: 'Characters a normal keyboard cannot type, rendered as tap-buttons beside the entries search box.' } } } } } }, responses: { 200: { description: '{ orthography }' }, 400: { description: 'Not found' } } },
      ...
      '/api/v1/dictionaries/{id}/conversations/{threadId}/questions': {
        ...
        post: { summary: 'Ask answerable questions', description: 'Files the whole-import questions from your report as objects the manager answers in the app. `kind` is `text` (open prose), `choice` (one option), or `multi_choice`; the choice kinds need >= 2 `options` — always include an escape option so "not sure" is answerable. `report_anchor` is an `id` in your report HTML, deep-linked from the question. **Attach `entries_query` wherever you can** — an entries-view filter object (`sources`, `dialects`, `speakers`, `tags`, `orthographies`, `parts_of_speech`, `semantic_domains`, `review_categories`, the `has_*`/`no_*` booleans, `query`) that renders as a "show me these entries" button, so the manager answers from the rows instead of from memory; an unknown filter key is a 400 rather than a silently wrong button, and `entries_query_label` is your own button wording ("Show me these 1,191 entries"). Keep `title` short and leave the long explanation in the report. Per-entry questions belong in the `review` queue instead. Appended after any existing questions. Team/API-key only.', parameters: [dict_id_param, conversation_id_param], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['questions'], properties: { questions: { type: 'array', items: { type: 'object', required: ['kind', 'title'], properties: { kind: { type: 'string', enum: ['text', 'choice', 'multi_choice'] }, title: { type: 'string' }, body_html: { type: 'string' }, options: { type: 'array', items: { type: 'object', required: ['value', 'label'], properties: { value: { type: 'string' }, label: { type: 'string' } } } }, report_anchor: { type: 'string' }, entries_query: { type: 'object', description: 'Entries-view filter naming the rows this question is about, e.g. { "sources": ["smith-1979"], "no_part_of_speech": true }.', additionalProperties: true }, entries_query_label: { type: 'string', description: 'Button wording, e.g. "Show me these 1,191 entries".' } } } } } } } } }, responses: { 200: { description: '{ questions }' }, 400: { description: 'Bad kind / missing title / choice question with < 2 options / unknown entries_query filter' }, 403: { description: 'Not the Living Dictionaries team' } } },
      },
