...
export function build_import_request_body({ origin, dictionary, requester, files, note, thread_id = '{threadId}' }: ImportRequestBodyOptions): string {
  ...
  return [
    ...
    '7. Finish (guide §2.8): POST the report HTML to …/conversations/{threadId}/artifacts and POST the whole-import questions to …/conversations/{threadId}/questions. Then DRAFT — do not post — the short warm conversation message and hand the draft to the human running the import: posting it is what emails the manager, and that send is a human decision. A human posts it and resolves the conversation.',
    '',
