...
export function build_import_request_body({ origin, dictionary, requester, files, note, thread_id = '{threadId}' }: ImportRequestBodyOptions): string {
  ...
  return [
    ...
    '7. Finish (guide §2.8): POST the report HTML to …/conversations/{threadId}/artifacts, POST the whole-import questions to …/conversations/{threadId}/questions, and POST a short warm message to …/conversations/{threadId}/messages — that message is what emails the manager. Then tell the Living Dictionaries team; a human resolves the conversation.',
    '',
