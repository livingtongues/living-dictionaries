...
/**
 * Tables admins READ via sync but never WRITE (defense in depth — the server
 * also strips writes). The upload loop in `engine.svelte.ts` skips these.
 *
 * `users` is download-only: the `users` table has no `dirty` column (admins
 * don't push user rows up — user edits go through endpoints like
 * `/api/me/profile`). Without this guard the upload loop runs
 * `SELECT * FROM "users" WHERE dirty = 1` and the whole sync fails with
 * "no such column: dirty".
 */
export const READONLY_TABLES: ReadonlySet<SyncableTableName> = new Set<SyncableTableName>(['users'])
