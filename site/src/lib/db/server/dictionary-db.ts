...
/** Absolute path of a dictionary's SQLite file (the file may or may not exist). */
export function dictionary_db_path(dict_id: string): string {
  return join(get_dictionaries_dir(), `${dict_id}.db`)
}
...
/**
 * Close any cached connection and delete the per-dict SQLite file on disk
 * (`<DATA_DIR>/dictionaries/<id>.db` + its -wal/-shm sidecars). Used by the
 * admin dictionary-teardown endpoint. Best-effort per file: a missing file is
 * not an error. Returns the list of paths that were actually removed.
 */
export function delete_dictionary_db_file(dict_id: string): string[] {
  close_dictionary_db(dict_id)
  const base = join(get_dictionaries_dir(), `${dict_id}.db`)
