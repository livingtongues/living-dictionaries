...
import type { FeaturedCard } from '$lib/components/home-v2/types'
...
export function list_featured_entries({ db, status }: { db: Database.Database, status?: FeaturedEntryStatus }): FeaturedEntry[] {
  if (status)
    return db.prepare('SELECT * FROM featured_entries WHERE status = ? ORDER BY created_at DESC').all(status) as FeaturedEntry[]
  return db.prepare('SELECT * FROM featured_entries ORDER BY created_at DESC').all() as FeaturedEntry[]
}
...
export function approved_featured_cards({ db }: { db: Database.Database }): FeaturedCard[] {
  const rows = db.prepare(`
    SELECT fe.id, fe.dict_id, COALESCE(d.url, fe.dict_id) AS dict_url, fe.dict_name, fe.entry_id,
    ...
    ORDER BY fe.created_at`).all() as FeaturedCard[]
  return rows
}
