...
export function parse_media_key(key: string): { dict_id: string, media_type: MediaLedgerType, is_variant: boolean } | null {
  const match = key.match(/^([^/]+)\/(audio|video|photo)\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}(_(?:thumb|w900|w1600|p1))?\.[\w-]{1,10}$/)
  if (!match)
    return null
...
if (import.meta.vitest) {
  ...
  test(parse_media_key, () => {
    ...
    expect(parse_media_key(`gta/audio/${uuid}_p1.mp3`)).toEqual({ dict_id: 'gta', media_type: 'audio', is_variant: true })
