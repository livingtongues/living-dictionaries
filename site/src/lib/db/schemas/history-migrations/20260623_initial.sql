-- Initial migration for the per-dictionary change-history db
-- (`dictionaries/{id}.history.db`). This file is SEPARATE from the main dict
-- db on purpose: history is server-only, never synced to clients and never
-- included in R2 viewer snapshots, so the main dict db + its snapshots stay
-- lean while history grows unbounded here.
--
