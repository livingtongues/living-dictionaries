...
export function build_personas({ real_admin_level }: { real_admin_level: number }): Persona[] {
  ...
  for (let level = real_admin_level; level >= 1; level--)
    personas.push({ key: `admin-${level}`, label: `Admin · level ${level}`, admin_level: level })
  if (real_admin_level >= 1)
    personas.push({ key: 'visitor', label: 'Visitor', admin_level: 0 })
  return personas
}
...
export function persona_label({ admin_level }: PreviewState): string {
  return admin_level >= 1 ? `Admin · level ${admin_level}` : 'Visitor'
}
...
if (import.meta.vitest) {
  describe(build_personas, () => {
    it('builds the full ladder for a level-2 admin', () => {
      expect(build_personas({ real_admin_level: 2 })).toEqual([
        { key: 'admin-2', label: 'Admin · level 2', admin_level: 2 },
        { key: 'admin-1', label: 'Admin · level 1', admin_level: 1 },
        { key: 'visitor', label: 'Visitor', admin_level: 0 },
      ])
    })

    it('starts the ladder at the real level (a level-1 admin cannot preview level 2)', () => {
      const personas = build_personas({ real_admin_level: 1 })
      expect(personas.map(persona => persona.key)).toEqual(['admin-1', 'visitor'])
    })
  ...
  describe(clamp_preview_level, () => {
    it('passes through a level at or below the real level', () => {
      expect(clamp_preview_level({ requested: 0, real_admin_level: 2 })).toBe(0)
      expect(clamp_preview_level({ requested: 1, real_admin_level: 2 })).toBe(1)
      expect(clamp_preview_level({ requested: 2, real_admin_level: 2 })).toBe(2)
    })

    it('clamps an escalation attempt down to the real level', () => {
      expect(clamp_preview_level({ requested: 2, real_admin_level: 1 })).toBe(1)
  ...
  describe(persona_label, () => {
    it('labels admin levels', () => {
      expect(persona_label({ admin_level: 2 })).toBe('Admin · level 2')
    })
