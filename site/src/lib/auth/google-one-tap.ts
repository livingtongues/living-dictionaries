 ...
 * element. Suitable for the `/login` route's auth form and the login modal.
 *
 * Always `theme: 'outline'` (white). GSI's button is a cross-origin iframe we
 * can't restyle, and only the iframe variant shows the personalized
 * "Sign in as <name>" face — which is the UX we want. To make the white button
 * mesh, the auth UI is forced to LIGHT theme (see `class="light"` on the login
 * Modal + `/login` card) rather than swapping to `filled_black`.
 */
