...
const shots = [
  ...
  { name: 'entries-list', url: '/dev/entries', wait: 5000, login: true },
  { name: 'entries-list-narrow', url: '/dev/entries', wait: 5000, width: 480, login: true },
  // the view lives inside the `q` JSON param (query-param-state store) — bare `?view=` is ignored
  { name: 'entries-table', url: `/dev/entries?q=${encodeURIComponent('{"view":"table"}')}`, wait: 5000, login: true },
  { name: 'entries-gallery', url: `/dev/entries?q=${encodeURIComponent('{"view":"gallery"}')}`, wait: 5000, login: true },
  { name: 'entries-print', url: `/dev/entries?q=${encodeURIComponent('{"view":"print"}')}`, wait: 5000, login: true },
  { name: 'entry-detail', url: '/dev/entry/06Tmb3jM1atoGNQvlxIY', wait: 4500, login: true },
  { name: 'entry-detail-narrow', url: '/dev/entry/06Tmb3jM1atoGNQvlxIY', wait: 4500, width: 480, login: true },
  { name: 'dict-about', url: '/dev/about', wait: 3000, login: true },
  { name: 'dict-contributors', url: '/dev/contributors', wait: 3000, login: true },
  { name: 'dict-settings', url: '/dev/settings', wait: 3500, login: true },
  { name: 'dict-export', url: '/dev/export', wait: 3000, login: true },
  { name: 'dict-import', url: '/dev/import', wait: 3000, login: true },
  { name: 'dict-grammar', url: '/dev/grammar', wait: 3000, login: true },
]
