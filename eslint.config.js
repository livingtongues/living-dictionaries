...
export default tseslint.config(
  // ─────────────────────────────────────────────────────────────
  // 1. Ignores — everything except the active site/ + root config
  // ─────────────────────────────────────────────────────────────
  {
    ignores: [
      '**/.svelte-kit/**',
  ...
  ...svelte.configs.recommended,
  {
    ...
    // LD's site is at livingdictionaries.app (apex). No `paths.base` to worry about.
    rules: {
  ...
  },
  {
    ...
    rules: {
      ...
      'no-useless-assignment': 'off', // misfires on $bindable/$props defaults (can't model two-way binding)
      'no-unused-vars': 'warn',
      'prefer-const': 'off',
  ...
  // Build-time-baked homepage data (generated by scripts/fetch-homepage-baked.mjs
  // from real dictionary content — lexemes/glosses can contain non-breaking spaces
  // and other intentional Unicode whitespace).
  {
    name: 'ld/homepage-baked',
    files: ['site/src/lib/data/homepage-baked.json'],
    rules: {
      'no-irregular-whitespace': 'off',
    },
  },
