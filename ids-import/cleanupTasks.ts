function adjustSemanticDomains(sheet_info: SheetData): void {
  ...
  (english_gloss_tsv_column_values as any[]).forEach((cell, i) => {
    const matchIndex = (english_gloss_sd_column_values as any[]).findIndex((value) => {
      if (cell[0] === value[0]) {
        current_semantic_domain_index = i
        // @ts-expect-error - GoogleApsScript magic
