...
type AllSpeakerData = Record<string, { supabase_id: string, speaker: TablesUpdate<'speakers'> }>
...
export async function migrate_entries(entries_to_test: any[], speakers: AllSpeakerData) {
  ...
  for (const fb_entry of entries_to_test) {
    const [_, supa_data] = convert_entry(fb_entry)
    ...
    for (const audio of audios) {
      ...
      if (new_speaker_name) {
        ...
        if (!new_speaker_id) {
          const { data, error: speaker_error } = await upsert_speaker({ dictionary_id, speaker: { name: new_speaker_name }, import_id })
          if (speaker_error)
    ...
    for (const dialect of dialects) {
      ...
      const { error: assign_error } = await assign_dialect({ dictionary_id, dialect_id, entry_id, import_id })
      if (assign_error) {
        throw new Error(assign_error.message)
      }
    }
    ...
    for (const sentence of sentences) {
      ...
      const { error } = await upsert_sentence({ dictionary_id, sense_id, sentence, sentence_id: sentence.id, import_id })
      if (error)
        throw new Error(error.message)
    }
