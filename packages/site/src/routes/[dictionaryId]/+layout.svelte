<script lang="ts">
  ...
  export let data;
  $: ({dictionary, admin, is_manager, search_index_updated} = data)
</script>
  ...
  <div class="flex px-3 print:px-0">
    <ResponsiveSlideover
      ...
      <div
        ...
        <SideMenu dictionary={$dictionary} admin={$admin} is_manager={$is_manager} search_index_updated={$search_index_updated} on_close={() => set(false)} />
        <hr class="md:hidden" />
