...
export function expand_entry(database_entry: GoalDatabaseEntry, t: TranslateFunction, storage_bucket: string): ExpandedEntry {
  const expanded_entry: ExpandedEntry = {
    ...
    senses: database_entry.sn?.map(sense => expand_sense(sense, t, storage_bucket)) || [{}],
    ...
    sound_files: database_entry.sfs?.map(audio => expand_audio(audio, storage_bucket)),
...
function expand_sense(sense: DatabaseSense, t: TranslateFunction, storage_bucket: string): ExpandedSense {
  const expanded_sense: ExpandedSense = {
    ...
    photo_files: sense.pfs?.map(photo => expand_photo(photo, storage_bucket)),
    video_files: sense.vfs?.map(video => expand_video(video, storage_bucket)),
    noun_class: sense.nc,
...
function expand_photo(pf: GoalDatabasePhoto, storage_bucket: string): ExpandedPhoto {
  const expanded_photo: ExpandedPhoto = {
    fb_storage_path: pf.path,
    storage_url: `https://firebasestorage.googleapis.com/v0/b/${storage_bucket}/o/${encodeURIComponent(pf.path)}?alt=media`,
...
export function expand_video(vf: GoalDatabaseVideo, storage_bucket: string): ExpandedVideo {
  const expanded_video: ExpandedVideo = {
    fb_storage_path: vf.path,
    storage_url: `https://firebasestorage.googleapis.com/v0/b/${storage_bucket}/o/${encodeURIComponent(vf.path)}?alt=media`,
    uid_added_by: vf.ab,
...
function expand_audio(sf: GoalDatabaseAudio, storage_bucket: string): ExpandedAudio {
  const expanded_audio: ExpandedAudio = {
    fb_storage_path: sf.path,
    storage_url: `https://firebasestorage.googleapis.com/v0/b/${storage_bucket}/o/${encodeURIComponent(sf.path)}?alt=media`,
    uid_added_by: sf.ab,
