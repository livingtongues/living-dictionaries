...
7. **The cutover** (`scripts/supabase-cutover/`) reads the same migration `*.sql` files off disk, so
   the new table's DDL is picked up automatically; only add data-population if the table has source
   data. `set_last_modified_to_max({ tables: [...] })` must include the new table.
