...
Decisions/gotchas from replacing the legacy client-stub READ path with real server
...
LD's data used to flow through a **client-side legacy-platform stub** threaded via *universal*
...
- **Entries**: a single **bundle endpoint** (`/api/dictionaries/[id]/entries-data`) that the
  worker fetches once, replacing the per-table legacy paging. We dropped the `cached_data_table`
...
data" lean) gives a real catalog + real entries for those 4. **achi (the e2e flow dict) started with 0
entries** — it now has a committed content fixture (`site/scripts/seed-achi-fixture.ts` + the
example repo's `.data`); real dict content can also be pulled from the VPS. Seed clean
...
## Projections (SQLite rows → legacy row shapes, so components don't change)
...
with *real* production data would NOT keep the flow "unchanged" (assertions are fixture-specific).
