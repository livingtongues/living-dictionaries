...
- **Media:** audio/video bytes live on **R2** (`livingdictionaries-media`, public via `media.livingdictionaries.app`; keys `{dict}/{audio|video}/{row_uuid}.{ext}`, migrated from GCS 2026-07-23 — see `.issues/media-r2-migration.md`). Uploads are presigned PUT via `/api/upload`; serving is dual-read in `src/lib/utils/media-url.ts`: new-convention paths → R2, legacy paths → GCS (which stays up as failsafe — don't tear down). **Photos are still on GCS** (Phase 2 pending) with App Engine Images `lh3` serving URLs (see `.knowledge/domain/media-serving-urls.md`). In dev there's no bucket: uploads are served from a local `/api/dev-media` store, while pulled photos still hit the public lh3 CDN.
