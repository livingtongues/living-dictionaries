...
- **Styling**: the end goal is hand-written scoped CSS + a small global layer driven by CSS custom properties; `unplugin-icons` (`~icons/*`) for icons, but we are currently in transition. The root layout (`src/routes/+layout.svelte`) globally imports `./reset.css` (Tailwind-style reset loaded into a low-priority `layer(reset)`), `virtual:uno.css`, then `$lib/theme.css` (semantic vars + body paint + Inter) and `$lib/buttons.css` (`.btn-*`). Those last two were the tutor-style admin-scoped theme promoted app-wide so the shared svelte-pieces resolve their `var(--…)` everywhere — they are no longer fenced to `.admin-theme`/`.admin-root`; dark mode is currently **light-forced site-wide**. The rest of the app still uses **UnoCSS** (purple utilities + presetIcons `i-*` + Font Awesome). Finishing the migration + dropping Uno (swapping uno's reset for the plain `archive/css/reset.css`) is a runbook in `.issues/app-group-removal-and-global-theme.md`.
