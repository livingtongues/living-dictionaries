...
- **Media:** ALL media bytes (audio/video/photos) live on **R2** (`livingdictionaries-media`, public via `media.livingdictionaries.app`; keys `{dict}/{audio|video|photo}/{row_uuid}.{ext}`). Audio/video upload via presigned PUT (`/api/upload`); audio originals stay canonical while a queued two-pass ffmpeg pipeline creates universal playback derivatives (`{uuid}_p1.mp3`, VBR V6/32 kHz mono, linear loudness gain, entry-only adaptive trim) with a post-upload ping (`$lib/server/audio-derivative.ts`) as THE conversion path; playback falls back to the original until the derivative exists. A ONCE-DAILY backfill catches what upload missed, and it runs in a forked `nice 19` CHILD (`$lib/db/server/audio-derivative-backfill.ts` does the work + is the child entry, `audio-derivative-sweep.ts` forks it and owns the ledger writes) — it was a 5-minute IN-PROCESS cron for one day and took the typical worst event-loop stall from 65 ms to 623 ms. Photos POST bytes to `/api/photo-upload`, which stores the original, responds, then generates 3 WebP variants (`_thumb/_w900/_w1600.webp`, `$lib/server/photo-variants.ts`) in-process. iPhone HEIC is converted to JPEG IN THE BROWSER (`$lib/media/prepare-image-upload.ts`): native `createImageBitmap` first (Safari only), then a libheif wasm decoder `import()`ed ONLY after the chosen file's leading bytes sniff as HEIC — a JPEG upload downloads none of it. Site-owned images use immutable `site/{asset}/{content_hash}/…` keys. Byte sizes live in the server-only shared.db `media_objects` ledger + `media_storage_daily` rollup (feeds `/admin/storage` — never lists R2 per page view); the sweep that maintains them reconciles weekly against a full R2 listing and REALLY deletes orphans past 30d grace (safe: the 1-year-locked `livingdictionaries-backups/media/` mirror). Like the audio backfill it runs in a forked `nice 19` CHILD (`$lib/db/server/media-sweep-child.ts` does the work + is the child entry and opens shared.db itself; `media-sweep-cron.ts` forks it and owns ALL telemetry) — in-process it listed 381,719 objects and froze the event loop 20.2 s (2026-08-06), and the object count grows weekly; `media_sweep_reconciled` now carries `duration_ms` + `step_ms {list,ledger_diff,heal}` + `blocking_ms`. Photo uploads have a 45 s timeout on the storage write (→ our own 504, not Cloudflare's 524) plus a `photo_upload_completed` success row with stage timings. In dev there's no bucket: uploads and derivatives are served from a local `/api/dev-media` store. See `.knowledge/domain/media-serving-urls.md`.
- **Search**: local **Orama** index built from the wa-sqlite DB.
- **Telemetry:** browser + server logs POST to `/api/log` → `client_logs`, which since 2026-07-05 lives in its own server-only **`logs.db`** (`src/lib/db/server/logs-db.ts`, split out of shared.db at boot; aged rows → `logs-archive.db`). The forever rollups `log_daily_metrics` + `log_daily_sessions` + `dictionary_daily_views` (per-dict daily distinct-session counts) + `dictionary_monthly_visitors` (TRUE monthly-unique visitors — a whole-month UNION of cookieless `visitor_id`s per dict AND a site-wide `__site__` scope, recomputed from raw each sweep then frozen so it outlives the 60d prune; feeds the "Top dictionaries by unique visitors" panel + the future public "visitors/month" badge) stay in `shared.db`, never pruned. **`/admin/analytics` + `/admin/health` run ZERO queries** (2026-07-30): they read a daily JSON CHECKPOINT from `${DATA_DIR}/analytics/<days>-<audience>.json`, written by a `nice -n 19` CHILD PROCESS the daily (03:30 PT) retention cron forks (`src/lib/db/server/analytics-snapshot.ts` — same shape in house + tutor). `get_log_analytics` (3-tier: shared.db rollups ≤ the retention watermark, live `logs.db` scans for the tail) now runs ONLY in that child, once a day per audience, and must never be called from a route — it measured 11–80 s of blocked event loop when it did. Neither raw-log file is backed up — see the **check-logs** skill. **Ingest normalizes two things** (2026-08-07, `insert-client-log.ts`): the boot-error reporter in `app.html` can only read SvelteKit's djb2 HASH of the build off the page shell (the bundle that knows the real name is what failed to load), so `resolve_app_version` swaps THIS build's hash back to the real version and keeps the raw hash in `context.version_hash` — without it every blank boot was permanently filed as a STALE build; and `context.is_bot` is stamped server-side from the UA via `is_bot_user_agent`, since those rows carry no session id for the session-level bot filters to reach. **Both error hooks report** (2026-08-01): `hooks.server.ts`'s `handleError` writes a `crash` row stamped with a short `error_id` that also travels to the browser as `App.Error.error_id`, and `hooks.client.ts` logs `client_error: …` for anything SvelteKit catches in its own browser-side load/render path — so `+error.svelte`'s crash row carries `origin: 'server' | 'client'` and either the joining `error_id` or the real cause. Until then a browser-raised 500 was indistinguishable from an SSR one (see `.knowledge/server/sveltekit-error-hooks.md`). **`monthly_metrics` (2026-08-01)** — one FROZEN shared.db row per month of the numbers we actually report: reach, the mission (`public`+`unlisted`) vs fenced (`conlang`+`glossary`) split, and corpus production split **agent vs hand** (`changes.api_key_id` in each dict's `.history.db`: non-NULL = an `/api/v1` agent, NULL = a human in the UI). Computed in the SAME niced analytics child — its only write — because the raw logs behind it prune at 60d. Windows are explicit (`window_start`/`window_end`/`days_counted`) since `visitor_id` only shipped 2026-07-07: July measures 07-08→07-31 and normalizes FORWARD via `normalize_visitors()` (uniques are a UNION — never scale them linearly), every later month a no-op. The retention cron's after-sweep then posts a short summary into the admin chat `notifications` room (`monthly-metrics-announce.ts`, idempotent via `announced_at`).
...
**PROBE BEFORE YOU ACCUSE (2026-08-07).** The immutable-asset ARCHIVE serves the previous builds'
assets for 30 days, which destroyed the "a failed chunk load is a deleted file BY CONSTRUCTION"
premise this rule was written on: on 2026-08-06 it fired 19 terminal verdicts over files that
answered `200`, and only 3 rows in the whole family that day were genuine. `classify_boot_failure`
now issues ONE `HEAD` against the artifact and only `404`/`410` is terminal — any other answer, an
incomplete probe, or being offline all keep the normal ladder. The URL comes from the message where a
browser names one, else from `leader_worker_url` (`db-client.ts` imports the worker via
Vite's **`./leader-worker.ts?worker&url`** rather than `new Worker(new URL(…))` precisely so the URL
is knowable — a worker `error` event carries nothing but `{ type: 'error' }`). Where no URL exists
(Safari names no module) it falls back to the old message patterns. Every boot-failure and terminal
row carries `script_url` / `online` / `probed_url` / `probe_status` / `probe_verdict`, so the verdict
is auditable. **Any new archive-fronted classifier must revisit its base rate the same way.**
